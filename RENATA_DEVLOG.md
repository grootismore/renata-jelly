...
## Phase 4 — Renata Browse Experience

Scope: Libraries landing, individual library browsing (grid + filters), Favorites, Search. Details and Player explicitly excluded. Working baseline confirmed at `56fad12`.

### 1. Audit (before any code changed)

**Libraries landing** — `app/(auth)/(tabs)/(libraries)/index.tsx` → `components/library/Libraries.tsx` (mobile) / `TVLibraries.tsx` (TV, not touched). Data: one `useQuery(["user-views", user.Id])` via `getUserViewsApi().getUserViews()`, filtered by `settings.hiddenLibraries` and to exclude `books`. Rendered as a single-column `FlashList` of `LibraryItemCard`, which itself branches on the pre-existing `settings.libraryOptions` (`display: "row"|"list"`, `imageStyle: "poster"|"cover"`, `showTitles`, `showStats` — all real, already-wired user settings surfaced via the Library tab's `...` menu in `_layout.tsx`, itself built on `PlatformDropdown`, a genuine native `UIMenu`/bottom-sheet component). Default settings (`display: "list"`, `imageStyle: "cover"`) render a landscape card with a dimmed backdrop image and title/stats overlay — already close to "tasteful landscape card presentation," just under-styled. **Bug found**: `LibraryItemCard` returned `null` outright when a library had no primary image (`if (!url) return null`) — a library with no artwork silently vanished from the list. **Bug found**: `Libraries.tsx`'s "no libraries" branch checked `if (!libraries)`, but `libraries` is always at least `[]` (via `?? []`), so that branch could never fire — zero libraries rendered a blank `FlashList` with no empty state.

**Individual library** — `app/(auth)/(tabs)/(libraries)/[libraryId].tsx`, ~1150 lines, mobile and TV in one file behind `Platform.isTV` branches (TV verified untouched by this phase). Mobile: `FlashList` with `numColumns` from a hand-rolled breakpoint ladder (`2/3/5/6/7/6` — note the drop from 7 to 6 above 1500pt, an apparent oversight), `renderItem` an inline `TouchableItemRouter` + alignSelf-based gutter trick + `ItemPoster`/`ItemCardText`, `ListHeaderComponent` a horizontal row of 7 `FilterButton`s/`ResetFiltersButton` (Reset, Genre, Year, Tags, Sort By, Sort Order, Filter By — each its own bottom-sheet via `FilterButton` → `useGlobalModal` → `FilterSheetContent`), full-screen `Loader` while loading, plain "no results" text when empty. Filter/sort state: `sortByAtom`/`sortOrderAtom`/`filterByAtom`/`genreFilterAtom`/`yearFilterAtom`/`tagsFilterAtom` (Jotai, global, restored per-library on focus from `*PreferenceAtom` maps keyed by `libraryId`) driving a single `getItemsApi().getItems()` call inside `useInfiniteQuery`. Favorites' "See all" (`app/(auth)/(tabs)/(favorites)/see-all.tsx`) is a near-duplicate of this grid — its own `2/3/5/6` breakpoint ladder, its own inline `renderItem` with the same alignSelf trick, `ItemPoster`/`ItemCardText`, full-screen `Loader`, plain-text empty state — fetching one favorite type via `getItemsApi().getItems({filters:["IsFavorite"], includeItemTypes:[itemType]})`.

**Favorites landing** — `app/(auth)/(tabs)/(favorites)/index.tsx` → `components/home/Favorites.tsx`, which renders one `InfiniteScrollingCollectionList` per favorite type (Series/Movies/Episodes/Videos/BoxSets/Playlists) — **the same shared Home component from Phase 3**, each `hideIfEmpty` with a "See All" → `see-all.tsx`. A hand-rolled empty state (heart PNG asset, tinted, + two `<Text>`s) covers the "every type empty" case.

**Search** — `app/(auth)/(tabs)/(search)/index.tsx`, ~960 lines. Uses the **native iOS/Android header search bar** (`navigation.setOptions({ headerSearchBarOptions })`) — not a custom on-screen field; this is already the "prominent but restrained," native-feeling pattern the phase asks for, so it was preserved as-is. Nine parallel `useQuery`s (movies/series/episodes/collections/actors/artists/albums/songs/playlists), each a direct Jellyfin `getItemsApi().getItems({ searchTerm, includeItemTypes })` (or a Streamystats/Marlin variant behind `settings.searchEngine`), 200ms-debounced. Results rendered via `SearchItemWrapper` (one per type, horizontal `FlashList`) with **nine separate inline `renderItem` implementations** — movies/series/collections/actors each hand-rolled `MoviePoster`/`SeriesPoster` + duplicate `<Text>` captions (functionally identical to what `ItemPoster`+`ItemCardText` already do for Library/Favorites); artists/albums/songs/playlists each hand-rolled a circular/square image box with an **emoji fallback** (👤/🎵/🎶) instead of the app's actual icon-based fallback convention (`ItemImage`'s `image-outline` Ionicon, used everywhere else). Episodes correctly used `ContinueWatchingPoster` (landscape, thumb-aware) — left as-is, it's the right presentation. Own `LoadingSkeleton` component (hand-rolled gray boxes + a Lorem-ipsum-as-background-color hack for text placeholders) instead of the Phase 2 `Skeleton` primitive. "No results" and the empty-query "example searches" list were plain `<Text>`. Discover/Jellyseerr tab (`SearchTabButtons`, `DiscoverFilters`, `JellyserrIndexPage`) already uses native SwiftUI glass buttons on iOS — left untouched, out of this phase's explicit scope.

**Shared data paths confirmed**: Library/Favorites-see-all/Search-library-results all ultimately call `getItemsApi().getItems()` with type/favorite/search filters — none of that was touched. `TouchableItemRouter` (routing table) — untouched. `ItemPoster`/`ItemCardText`/`WatchedIndicator`/`ItemImage` — the real shared primitives, already used by Library and (via `InfiniteScrollingCollectionList`) Favorites; Search was the outlier duplicating them ad hoc.

### 2. Shared primitives introduced (§F)
- **`utils/getResponsiveGridColumns.ts`** (new) — `columns = clamp(round(screenWidth / 110), 2, 10)`. Targets a constant ~110px card width (matching the `w-28`/112px poster convention already used across Home/Search rows) instead of a fixed breakpoint ladder, so wider screens (iPad) naturally get *more columns* rather than *bigger cards* (§4/§14's explicit ask). Replaces both the library grid's and Favorites-see-all's separate, slightly different breakpoint ladders — including the 7→6 regression above 1500pt noted in §1, which this formula doesn't have (documented under §N as a side effect of the requested redesign, not a silent fix).
- **`components/library/LibraryGridItem.tsx`** (new) — the shared grid card (`TouchableItemRouter` + the existing alignSelf gutter trick, unchanged, + `ItemPoster` + `ItemCardText`), extracted from the two near-duplicate `renderItem` implementations in `[libraryId].tsx` and `favorites/see-all.tsx`. Both screens now render identically.
- **`components/library/LibraryGridSkeleton.tsx`** (new) — a grid of `Skeleton` blocks sized to `ItemPoster`'s 10:15 aspect + two caption lines, parameterized by the same column count as the real grid. Used by both grid screens in place of the full-screen spinner.
- **`components/posters/ItemPoster.tsx`** (small addition) — `Person` added to the portrait-aspect condition (alongside `Movie`/`Series`/`BoxSet`); it previously fell through to the square branch, which is wrong for a person photo. Enables reusing `ItemPoster` for Search's Actors row. Checked all four call sites (`(search)/index.tsx`, `(watchlists)/[watchlistId].tsx`, `collections/[collectionId].tsx`, and the new `LibraryGridItem`) — none of the other three currently pass `Person` items through it, so this is a pure widening with no observed behavior change elsewhere.
- **`components/common/ItemImage`** (existing, Phase-2-era, unmodified) — reused directly for Search's artists/albums/songs/playlists art instead of each hand-rolling its own `<Image>` + emoji-fallback box. Gives those rows the same icon-based empty-artwork fallback as the rest of the app.
- **`components/common/EmptyState` / `ErrorState`** (Phase 2, unmodified) — adopted by Libraries landing, individual library grid, Favorites landing, Favorites-see-all, and Search's no-results/no-type states, replacing five different hand-rolled "no items" `<Text>` blocks and one hand-rolled heart-icon empty state.
- **No new component was created** for anything `ItemPoster`+`ItemCardText`(+`ItemImage` for the fallback) could already do — per §8, audited before writing anything and found the existing primitives sufficient once `ItemPoster` gained the one-line `Person` fix.

### A. Library landing changes
`components/library/LibraryItemCard.tsx`'s default ("cover") and row-fallback ("list"+"poster") variants restyled onto Renata tokens (`Radius.lg`, `Surface.elevated`/`border`, `TextColor` hierarchy) instead of raw `neutral-900`/hardcoded hex. The "cover" card gained a bottom-anchored gradient (restrained, not Home's cinematic treatment — a single `LinearGradient` stop pair) for title/stat legibility, and an **elegant fallback** (a type-appropriate `Ionicons` glyph on a `Surface.elevated`/`Surface.page` background) for libraries with no primary image, replacing the silent `return null` bug from §1. `Libraries.tsx` gained `isError`→`ErrorState`, a 3-row `Skeleton` loading state (replacing the full-screen spinner), and a real `libraries.length === 0` → `EmptyState` branch (replacing the dead `!libraries` check from §1). All existing navigation (`TouchableItemRouter`), the `row`/`list` × `poster`/`cover` display-mode branching, and the `showTitles`/`showStats` toggles are unchanged — same settings, same behavior, just restyled.

### B. Individual-library/grid changes
`[libraryId].tsx`'s mobile grid now uses `getResponsiveGridColumns` (§2) instead of its own breakpoint ladder, and `LibraryGridItem` instead of its inline `renderItem`. Loading → `LibraryGridSkeleton` (sized to the live column count); a genuine fetch error → `ErrorState` with retry (new — the query's `isError` was already available, just not surfaced); empty results → `EmptyState`. TV's rendering (`renderTVItem`, the `flexWrap` grid, the TV filter bar) is a completely separate code path behind `Platform.isTV` and was not touched — verified via re-reading that branch after editing and via `git diff`.

### C. Sorting/filter changes
Purely visual. `FilterButton.tsx` and `ResetFiltersButton.tsx` restyled onto Renata tokens (`Surface.elevated`/`border` for inactive, `Colors.primary` for the active/selected state — "subtle accent usage" per §2) with smaller padding/icon sizes; the `ListHeaderComponent` row's padding tightened (16/10 instead of 15/16) for a less vertically heavy strip. **Zero changes** to `FilterButton`'s underlying mechanism (still fetches its own options via `useQuery`, still opens `FilterSheetContent` in a bottom sheet, still calls the same `set()` callbacks wired to the same `*PreferenceAtom` persistence), the sort/filter Jotai atoms, the `useFilterReset` hook, or the `getItems()` call's filter/sort parameters. No new dependency — reused the existing `FilterButton`/`FilterSheetContent`/`PlatformDropdown` architecture rather than introducing a new menu system.

### D. Favorites changes
Landing (`components/home/Favorites.tsx`): swapped the hand-rolled heart-PNG empty state for `EmptyState` (§6's explicit instruction); the six `InfiniteScrollingCollectionList` rows (Series/Movies/Episodes/Videos/BoxSets/Playlists), their queries, pagination, `hideIfEmpty`, and "See All" navigation are byte-identical to before. "See all" (`see-all.tsx`): same treatment as the library grid (§B) — shared `LibraryGridItem`/`getResponsiveGridColumns`/`LibraryGridSkeleton`/`EmptyState`/`ErrorState`, zero changes to the favorites query (`filters:["IsFavorite"]`, `includeItemTypes`, pagination) or to `useFavorite`/Jellyfin favorite state anywhere in the app. No "My List" concept was introduced — Jellyfin Favorites remains the only saved-content mechanism, exactly as instructed.

### E. Search changes
Native header search bar, debouncing, and all nine query definitions (Jellyfin/Streamystats/Marlin search engines, music search) — untouched. Movies/Series/Collections/Actors rows now render via `ItemPoster`+`ItemCardText` (via `SearchItemWrapper`, which now uses the shared `SectionHeader` instead of its own `<Text>` header) instead of duplicated `MoviePoster`/`SeriesPoster`+manual caption markup — same visual family as Library/Favorites, per §7's explicit instruction. Episodes unchanged (`ContinueWatchingPoster`, already correct). Artists/Albums/Songs/Playlists keep their type-appropriate shapes (circular for artists, square for the rest) but the image itself is now `ItemImage` (consistent icon fallback instead of emoji). `LoadingSkeleton.tsx` rebuilt on the shared `Skeleton` primitive. "No results" → `EmptyState` (icon `search-outline`, message the quoted query); the empty-query "example searches" list restyled onto `TextColor` tokens, same tap-to-search behavior. Discover/Jellyseerr tab untouched.

### G. iPhone/iPad responsive behavior
`getResponsiveGridColumns` is the single mechanism now driving both grid screens: it recomputes from live `screenWidth` (already tracked via `useWindowDimensions`), so rotation and iPad multitasking resizes are handled the same way they already were (both screens already re-render on width change; nothing new added there). Because the formula targets a constant card width rather than a step function tied to iPhone-sized breakpoints, an iPad simply resolves to a larger `columns` value automatically — more, similarly-sized cards, not enlarged iPhone cards. Search's rows remain horizontal scrollers (unchanged) — appropriately, since they're results grouped by type, not a single grid; iPad users get the same "more visible at once" scaling for free from the wider viewport without any explicit iPad-specific code.

### H. Loading/empty/error behavior
Every redesigned screen now has three explicit states — loading (`Skeleton`/`LibraryGridSkeleton` sized to the real geometry), empty (`EmptyState`), and error (`ErrorState`, with retry where a `refetch` was available) — replacing five separate "spinner → blank → grid" sequences and hand-rolled empty-text blocks. Pagination footers (`ListFooterComponent` showing `Loader` while fetching the next page) are unchanged in both grid screens — only the *initial* load state changed, per §10's explicit "don't block an entire screen because another page is loading."

### I. Data/query semantics preserved
Confirmed for every redesigned screen, before → after:
- Libraries landing: same `getUserViewsApi().getUserViews()` call, same `hiddenLibraries`/`books` filtering, same per-library item-count query.
- Individual library: same `getItemsApi().getItems()` call inside the same `useInfiniteQuery`, same `queryKey`, same sort/filter/genre/year/tag atoms and their per-library preference persistence, same pagination (`getNextPageParam`), same 36-item page size.
- Favorites (landing + see-all): same `filters:["IsFavorite"]`/`includeItemTypes`/sort (`SeriesSortName, SortName`) queries, same page sizes (20/50), same pagination.
- Search: all nine `useQuery`s, their `queryKey`s, the 200ms debounce, and the Jellyfin/Streamystats/Marlin engine switch are untouched.
- Watched status, favorite state, and item routing: still read via the same `item.UserData`/`useFavorite`/`TouchableItemRouter` — no redesigned component recomputes or duplicates any of this state.

### J. Protected playback infrastructure confirmation
`git status --short` shows changes only under `app/(auth)/(tabs)/(libraries)/`, `app/(auth)/(tabs)/(favorites)/`, `app/(auth)/(tabs)/(search)/`, `components/library/`, `components/filters/`, `components/home/Favorites.tsx`, `components/posters/ItemPoster.tsx`, `components/search/`, and the new `utils/getResponsiveGridColumns.ts`. Zero changes to `modules/mpv-player`, `MPVKit`, `PlayerEngine`, `PlaybackInfo`/device-profile code, Direct Play declarations, audio/subtitle behavior, progress reporting, downloads/offline playback, Jellyfin authentication, WebSocket architecture, or `hooks/usePlaybackEntry.ts` (Phase 3.1). No item-details or player screen was touched.

### K. Validation results
- `bun run typecheck` ✅ pass.
- `bun run check` (biome) — one real bug caught by the linter itself: `components/home/Favorites.tsx` had a local `type EmptyState = Record<FavoriteTypes, boolean>` that collided with the newly-imported `EmptyState` component (`noRedeclare`) — renamed the local type to `FavoritesEmptyState`. Formatting-only diffs elsewhere, fixed via `bun run format` + `bun run lint`, then ✅ pass, 741 files.
...
- Modified: `app/(auth)/(tabs)/(libraries)/[libraryId].tsx`, `app/(auth)/(tabs)/(favorites)/see-all.tsx`, `app/(auth)/(tabs)/(search)/index.tsx`, `components/library/Libraries.tsx`, `components/library/LibraryItemCard.tsx`, `components/filters/FilterButton.tsx`, `components/filters/ResetFiltersButton.tsx`, `components/home/Favorites.tsx`, `components/posters/ItemPoster.tsx`, `components/search/SearchItemWrapper.tsx`, `components/search/LoadingSkeleton.tsx`.
- Not touched: Details, Player, Settings, TV (any `*.tv.tsx` file or `Platform.isTV` branch), navigation/`_layout.tsx` files, any protected path.

### N. Existing bugs discovered
- **Fixed as a direct consequence of the requested redesign** (not a silent, separate behavior change): `LibraryItemCard` rendering nothing for libraries with no primary image (§1) — now shows the elegant fallback §3 explicitly asked for. `Libraries.tsx`'s dead `!libraries` empty-state check (§1) — now a real `libraries.length === 0` check backed by `EmptyState`. The individual library grid's column formula dropping from 7 to 6 columns above 1500pt width (§1/§2) — superseded by the new width-proportional formula as a natural side effect of building the requested responsive grid, not a targeted fix.
- **Discovered, not fixed** (out of scope / lower confidence, flagged for a future pass rather than changed here): Favorites-see-all's alignSelf gutter trick previously applied its flex-end/flex-start/center logic unconditionally, while the individual library grid only applied it in portrait (centering everything in landscape). Unifying both onto `LibraryGridItem` means Favorites-see-all now also centers in landscape, matching the library grid's arguably-more-correct behavior — a minor, presentation-only inconsistency resolved by the shared component, not something requiring further action. `Search`'s "no results" state and the individual library's "no results" state use different existing translation keys (`search.no_results_found_for` vs. `library.no_results`) with slightly different phrasing conventions app-wide; left as-is since unifying copy across features is a content decision, not a Phase 4 presentation task.
