  ...
  const payloadItems = (items || [])
    ...
    .map((item) => {
      ...
      return {
        ...
        imageUrl: image?.url,
        route: `renata://topshelf/item?id=${encodeURIComponent(item.Id!)}&type=${encodeURIComponent(item.Type || "")}`,
        playRoute: `renata://topshelf/play?id=${encodeURIComponent(item.Id!)}`,
      };
    });
